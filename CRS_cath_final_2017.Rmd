...
anaerobes %>% filter(Disease_Status=="CRS") %>% ggplot(aes(x=Sinus2,y=sum_abund, fill=Genus))+geom_bar(stat="identity")+
  facet_wrap(~Patient, 7,7, scales="free")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  scale_fill_brewer(palette="Set1")+labs()

anaerobes %>% filter(Disease_Status=="CONTROL") %>% ggplot(aes(x=Sinus2,y=sum_abund, fill=Genus))+geom_bar(stat="identity")+
...
indplot<-cor_otu_data %>% filter(OTU %in% c("410908","1075821")) %>%
  ...
  labs(title="SNOT22 vs abundance",x="relative abundance (%)")+
  ylim(0,100)+facet_grid(.~Genus, scales="free")
