...
#Use ggplot for better plots:
sinus_alpha<-data.frame(estimate_richness(sinus2017_rare,measures=c("Observed","Chao1","Shannon")))
sinus_alpha<-cbind(sinus_alpha,sample_data(sinus2017_rare))
sinus_nostMM<-subset(sinus_alpha, Sinus %in% c("NOST", "MM"))
sinus_alpha_sinus<- subset(sinus_alpha, Sinus %in% c("ETH", "MAX", "SPH", "FRO"))
alpha_MM<-subset(sinus_alpha, Sinus=="MM")
sinus_alpha_CRS<-subset(sinus_alpha_sinus, Disease_Status=="CRS" & NP_Status!="NA")

sinus_alpha_plot<-ggplot(sinus_alpha_sinus,aes(x=Disease_Status, y=Shannon, fill=Disease_Status))+geom_boxplot()+theme(legend.position='none')+ylim(0,4)

CRS_alpha_plot<-ggplot(sinus_alpha_CRS,aes(x=NP_Status, y=Shannon))+geom_boxplot(aes(fill="#00BFC4"))+theme(legend.position='none')+ylim(0,4)+ scale_fill_manual(values="#00BFC4")

Figure1AB<-grid.arrange(sinus_alpha_plot, CRS_alpha_plot, ncol=2)
...
#Now MM only
MM_only<- subset_samples(sinus2017_rare, Sinus=="MM")
plot_richness(MM_only, x="Disease_Status", measures=c("Observed","Chao1","Shannon"))+
  geom_boxplot(aes(fill=Disease_Status))+
  xlab("Disease Status")+ylab("Diversity")+ggtitle("Alpha Diversity of MM According to Disease Status")
...
#Disease Status sinuses only
anova(lmer(Shannon ~ Disease_Status + (1|Patient), data=sinus_alpha_sinus))
rand(lmer(Shannon ~ Disease_Status + (1|Patient), data=sinus_alpha_sinus))
anova(lmer(Observed ~ Disease_Status + (1|Patient), data=sinus_alpha_sinus))
rand(lmer(Observed ~ Disease_Status + (1|Patient), data=sinus_alpha_sinus))
anova(lmer(Chao1 ~ Disease_Status + (1|Patient), data=sinus_alpha_sinus))
rand(lmer(Chao1 ~ Disease_Status + (1|Patient), data=sinus_alpha_sinus))
...
#now just with MM samples
anova(lmer(Shannon ~ Disease_Status + (1|Patient), data=alpha_MM))
rand(lmer(Shannon ~ Disease_Status + (1|Patient), data=alpha_MM))
anova(lmer(Observed ~ Disease_Status + (1|Patient), data=alpha_MM))
rand(lmer(Observed ~ Disease_Status + (1|Patient), data=alpha_MM))
anova(lmer(Chao1 ~ Disease_Status + (1|Patient), data=alpha_MM))
rand(lmer(Chao1 ~ Disease_Status + (1|Patient), data=alpha_MM))
...
library(tidyverse)
Patient_evenness<-mutate(sinus_alpha, evenness=Shannon/log(Observed))
...
#define a contrasting colour palette
mycol<-palette(c("#e6194b","#3cb44b","#d2f53c","#ffe119","#0082c8","#f58231","#911eb4","#46f0f0","#f032e6","#fabebe","#008080","#e6beff","#aa6e28","#fffac8","#800000","#aaffc3","#808000","#ffd8b1","#000080","#808080","#FFFFFF","#000000","plum1","dodgerblue","chartreuse","paleturquoise","magenta","darkred"))
...
phyla_plot<-sinus_plots %>% 
  ...
  filter(av_rel_abund>0.1)
  
fig1a<-ggplot(phyla_plot, aes(x=Patient, y=av_rel_abund, fill=Phylum)) + geom_bar(stat="identity")+facet_grid(.~Disease_Status, scales="free")+
  guides(fill=guide_legend(ncol=1))+theme(axis.text.x = element_text(angle = 90, hjust = 1),legend.key.size = unit(0.5,"line")) +ggtitle("Average phyla per patient in sinuses only")+scale_fill_manual(values=c("p__[Thermi]"="#e6194b","p__Actinobacteria"="#3cb44b","p__Bacteroidetes"="#ffe119","p__Chloroflexi"="#0082c8","p__Cyanobacteria"="#f58231","p__Firmicutes"="#911eb4","p__Fusobacteria"="#46f0f0","p__Proteobacteria"="#f032e6","p__Synergistetes"="#d2f53c","p__TM7"="#fabebe"))
...
ggplot(nost_mm_phyla, aes(x=Patient, y=av_rel_abund, fill=Phylum)) + geom_bar(stat="identity")+facet_grid(.~Disease_Status, scales="free")+
  guides(fill=guide_legend(ncol=1))+theme(axis.text.x = element_text(angle = 90, hjust = 1),legend.key.size = unit(0.5,"line")) +ggtitle("Average phyla per patient in nostril and MM only")+scale_fill_manual(values=c("p__[Thermi]"="#e6194b","p__Actinobacteria"="#3cb44b","p__Bacteroidetes"="#ffe119","p__Chloroflexi"="#0082c8","p__Cyanobacteria"="#f58231","p__Firmicutes"="#911eb4","p__Fusobacteria"="#46f0f0","p__Proteobacteria"="#f032e6","p__Synergistetes"="#d2f53c","p__TM7"="#fabebe"))
#Check out MM
mm_phyla <- nost_mm_plot %>% filter(Sinus=="MM") %>% 
  group_by(Patient,OTU) %>%
  mutate(av_rel_abund=mean(rel_abund))%>%
  distinct(Patient,OTU, .keep_all=TRUE) %>%
  filter(av_rel_abund>0.1) %>%
...
ggplot(mm_phyla, aes(x=Patient, y=av_rel_abund, fill=Phylum)) + geom_bar(stat="identity")+facet_grid(.~Disease_Status, scales="free")+
  guides(fill=guide_legend(ncol=1))+theme(axis.text.x = element_text(angle = 90, hjust = 1),legend.key.size = unit(0.5,"line")) +ggtitle("Average phyla per patient in MM only")+scale_fill_manual(values=c("p__[Thermi]"="#e6194b","p__Actinobacteria"="#3cb44b","p__Bacteroidetes"="#ffe119","p__Chloroflexi"="#0082c8","p__Cyanobacteria"="#f58231","p__Firmicutes"="#911eb4","p__Fusobacteria"="#46f0f0","p__Proteobacteria"="#f032e6","p__Synergistetes"="#d2f53c","p__TM7"="#fabebe"))

#Look at genera level
genus_plots<-sinus_plots %>% group_by(Sample,Genus)%>%
  mutate(sum_abund=sum(rel_abund)) %>%
  distinct(Genus) %>%
...
ggplot(genus_plots,aes(x=Patient, y=av_abund, fill=Genus))+geom_bar(stat="identity")+facet_grid(.~Disease_Status, scales="free")+
  guides(fill=guide_legend(ncol=1))+theme(axis.text.x = element_text(angle = 90, hjust = 1),legend.key.size = unit(0.5,"line")) +ggtitle("Average Genus abundance per patient in sinuses only")+scale_fill_manual(values=mycol)

mm_genera <- nost_mm_plot %>% filter(Sinus=="MM") %>% 
  ...
  filter(av_abund>1.5) %>%
  filter(Genus!="NA")
ggplot(mm_genera, aes(x=Patient, y=av_abund, fill=Genus))+geom_bar(stat="identity")+
  facet_grid(.~Disease_Status, scales="free")+scale_fill_manual(values=mycol)+labs(title="Average Genus abundance per patient in middle meatus samples only", y="Average relative abundance (%)")+
...
entero<-otu_smelt %>%
  mutate(rel_abund=Abundance/6000*100) %>%
  filter(Family=="f__Enterobacteriaceae") %>% filter(Genus=="g__")

E1<-entero %>% filter(Sinus=="MM") %>%
  group_by(Sample,Family) %>% mutate(sum_abund=sum(rel_abund)) %>%
  group_by(Sample,Patient)%>%
  distinct(Family, .keep_all=TRUE)%>%
  group_by(Patient)%>%
  mutate(av_abund=mean(sum_abund)) %>%
  distinct(Patient, .keep_all=TRUE) %>%
  ggplot(aes(x=Patient, y=av_abund, fill=Genus))+geom_bar(stat="identity")+
  facet_grid(.~Disease_Status, scales="free")+scale_fill_manual(values=mycol)+labs(title="Middle meatus samples", y=NULL, x=NULL)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1),legend.position="none",plot.title = element_text(size=10))+
  ylim(0,50)

E2<-entero %>% filter(Sinus!="MM") %>%
  group_by(Sample,Family) %>% mutate(sum_abund=sum(rel_abund)) %>%
  group_by(Sample,Patient)%>%
  distinct(Family, .keep_all=TRUE)%>%
  group_by(Patient) %>%
  ...
  distinct(Patient, .keep_all=TRUE) %>%
  ggplot(aes(x=Patient, y=av_abund, fill=Genus))+geom_bar(stat="identity")+
  facet_grid(.~Disease_Status, scales="free")+scale_fill_manual(values=mycol)+labs(title="Sinus samples", y=NULL)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1),legend.position="none",plot.title = element_text(size=10))+
  ylim(0,50)

grid.arrange(E1,E2,nrow=2, top="Unclassified Enterobacteriaceae relative abundance", left="Average relative abundance (%)")

#Look at Corynebacterium OTUs
top10_cory<-data.frame(otu_smelt %>% filter(Genus=="g__Corynebacterium") %>% group_by(OTU) %>% mutate(sum_otu=sum(Abundance)) %>% distinct(OTU, .keep_all=TRUE) %>% select(OTU,Genus, Species, sum_otu) %>% arrange(desc(sum_otu)))$OTU[1:10]

cory_otus<-otu_smelt %>% filter(Genus=="g__Corynebacterium") %>% filter(OTU %in% top10_cory) %>% mutate(rel_abund=Abundance/6000*100) %>% filter(Disease_Status!="NA")
ggplot(cory_otus, aes(x=Patient,y=rel_abund))+geom_boxplot()+facet_grid(OTU~Disease_Status, scales="free")+labs(title="Top 10 Corynbacterium OTU relative abudnance accross patients")



#Look at anaerobes
data.frame(genus_plots %>% arrange(Patient) %>% select(Patient,Genus, av_abund,Disease_Status) %>% filter(Genus %in% c("g__Actinomyces", "g__Dialister","g__Megasphera", "g__Parvimonas","g__Porphyromonas","g__Prevotella","g__Anaerococcus","g__Finegoldia", "g__Peptinophillus","g__Prevotella","g__Fusobacterium")) %>% arrange(Genus, desc(av_abund)))



anaerobes<-sinus_plots %>% group_by(Sample,Genus)%>%
  mutate(sum_abund=sum(rel_abund)) %>% select(Patient,Genus, sum_abund,Disease_Status,Sinus,Sample) %>% filter(Genus %in% c("g__Actinomyces", "g__Dialister","g__Megasphera","g__Parvimonas","g__Porphyromonas","g__Prevotella", "g__Anaerococcus","g__Finegoldia","g__Peptinophillus","g__Prevotella","g__Fusobacterium")) %>% arrange(Genus,desc(sum_abund)) %>% distinct(Genus, .keep_all=TRUE) %>% arrange(Sample,Patient)

anaerobes$Sinus2<-gsub("[0-9]+([A-Z]+)","\\1",anaerobes$Sample)

ggplot(anaerobes, aes(x=Sinus,y=sum_abund, fill=Genus))+geom_bar(stat="identity")+
  facet_grid(Patient~Disease_Status, scales="free")
library("RColorBrewer")

figureS2<-anaerobes %>% filter(Disease_Status=="CRS") %>% ggplot(aes(x=Sinus2,y=sum_abund, fill=Genus))+geom_bar(stat="identity")+
  facet_wrap(~Patient, 7,7, scales="free")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  scale_fill_brewer(palette="Set1")+labs(x="Sinus",y="Relative abundance %", title="Anaerobes in CRS Sinuses")+ylim(0,90)

figureS3<-anaerobes %>% filter(Disease_Status=="CONTROL") %>% ggplot(aes(x=Sinus2,y=sum_abund, fill=Genus))+geom_bar(stat="identity")+
  facet_wrap(~Patient, 3,4, scales="free")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  scale_fill_brewer(palette="Set1")+labs(x="Sinus",y="Relative abundance %", title="Anaerobes in Control Sinuses")+ylim(0,50)
...
sig_sinus_genus<- sinus_plots %>% 
  filter(Genus %in% c("g__Corynebacterium","g__Alloiococcus","g__Staphylococcus") | Family=="f__Enterobacteriaceae"&Genus=="g__") %>%
  mutate(rel_abund=Abundance/6000*100) %>%
  group_by(Genus,Sample)%>%
  mutate(sum_abund=sum(rel_abund))

ggplot(sig_sinus_genus, aes(x=Patient,y=sum_abund, fill=Disease_Status))+
  geom_boxplot() +
  facet_grid(Family~Disease_Status, scales="free")+labs(y="Relative Abundance (%)", title="Abundance of Significantly Different Genera in CRS and Controls") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

sig_sinus_genus %>% filter(Genus=="g__Corynebacterium") %>%
  ggplot(aes(x=Patient,y=sum_abund,fill=Disease_Status))+geom_boxplot() + 
...
sinus2017_rare_wuf<-phyloseq::distance(sinus2017_rare_fil2, method="wunifrac", type="samples")
...
sinus_wuf<-phyloseq::distance(Sinus_only_fil, method="wunifrac", type="samples")
...
sample_data(Sinus_only_fil)$Patient<-as.factor(sample_data(Sinus_only_fil)$Patient)
plot_ordination(Sinus_only_fil, ord_sinus_wuf,color="Disease_Status") + stat_ellipse()+labs(title="PCoA of sinus samples")
sample_data(Sinus_only_fil)$run_no<-as.factor(sample_data(Sinus_only_fil)$run_no)
#Test for differences in dispersion
bd<-betadisper(sinus_wuf,(sample_data(Sinus_only_fil))$Disease_Status, type="centroid")
permutest(bd)
adonis(sinus_wuf~Disease_Status/Patient + Patient + Sinus + run_no, data=data.frame(sample_data(Sinus_only_fil)))

#subset to sphenoid only and repeat
sphenoid_only_fil<-subset_samples(Sinus_only_fil, Sinus=="SPH")
sph_wuf<-phyloseq::distance(sphenoid_only_fil, method="wunifrac", type="samples")
sph_ord<-ordinate(sphenoid_only_fil, method="PCoA", distance=sph_wuf)
plot_ordination(sphenoid_only_fil, sph_ord, color="Disease_Status") +
  stat_ellipse() + labs(title="PCoA of spheonid sinus samples")

adonis(sph_wuf~Disease_Status/Patient + Patient + run_no, data=data.frame(sample_data(sphenoid_only_fil)))

bds<-betadisper(sinus_wuf,(sample_data(Sinus_only_fil))$Disease_Status)
permutest(bds)

plot_ordination(Sinus_only_fil, ord_sinus_wuf,color="run_no", shape="Disease_Status")
...
#plotting disease status and NP/run_no/PCR_no
plot_ordination(nost_mm_fil, ord_nost_wuf,color="Disease_Status")+ggtitle("Weighted unifrac PCoA from nost and MM swabs") + stat_ellipse()
...
distance_boxplot<-ggplot(patient_distances, aes(x=Disease_Status, y=distance, fill=Disease_Status))+geom_boxplot()+ ggtitle("Intra-patient Weighted Unifrac Distances") + xlab("Health Status") +ylab("Unifrac Distance") + theme(plot.title = element_text(hjust = 0.5)) +theme(axis.text=element_text(size=12),                                 axis.title=element_text(size=14)) + theme(plot.title = element_text(size = 16)) 

distance_boxplot2 <- distance_boxplot + scale_x_discrete(breaks = c("Contol", "CRS"), labels = c("Healthy","CRS")) + xlab(NULL) + theme(legend.position = "bottom", legend.background = element_rect(color = "white", fill = "white", size = 1, linetype = "solid"), legend.direction = "horizontal")
#Do this with the wunifrac distances matrices from phyloseq, only include sinus samples for comparisons - will do this below the next section and use the dataframe of distances I created.
...
intra_inter<-test2_m %>% mutate(comp_type=ifelse(P1==P2, "intra", "inter")) %>%
  filter(!S1 %in% c("NOST","LMM","RMM") & !S2 %in% c("NOST","LMM","RMM"))
anova(lmer(value ~ comp_type + (1|P2), data=intra_inter))

ggplot(intra_inter, aes(x=comp_type, y=value))+geom_boxplot()+facet_grid(.~D1, scales="free")
...
test<- test2_m %>% filter(!S1 %in% c("NOST","LMM","RMM") & !S2 %in% c("NOST","LMM","RMM")) %>%
  filter(P1==P2) %>% arrange(P1)
P1<-ggplot(test, aes(x=D1, y=value, fill=D1))+
  geom_boxplot()+labs(title="Within Patient Wieghted Unifrac Distances", x="Disease Status", y="Weighted unifrac distance")+
  theme(legend.title=element_blank())

  geom_point(position="jitter")
show_col(hue_pal()(4))  
P2<-test %>% filter(D1=="CONTROL") %>% ggplot(aes(x=P1, y=value))+
  geom_boxplot(fill="#F8766D")+labs(title="Within Control Patient Wieghted Unifrac Distances", y="Weighted unifrac distance", x="Patient")
P3<-test %>% filter(D2=="CRS") %>% ggplot(aes(x=P1, y=value))+
  geom_boxplot(fill="#00BFC4")+labs(title="Within CRS Patient Wieghted Unifrac Distances", y="Weighted unifrac distance", x="Patient")
grid.arrange(P1, arrangeGrob(P2,P3,ncol=1), ncol=2)
#Check for significance in difference between wunifrac intra patient distances
anova(lmer(value ~ D1 + (1|P2), data=test))
anova(lm(value ~ D1 + P2, data=test))
rand(lmer(value ~ D1 + (1|P2), data=test))
#Not sure if I need to have patient as a random variable here - I think I do, but this does make it non-significant. What if I average the values per patient, and test for sig dif between average distance within patients in CRS and Controls?
average_dist<- test %>% group_by(D1,P1) %>% summarise(av_dist=mean(value)) 
anova(lm(av_dist ~ D1, data=average_dist))

#Repeat above analysis of intrapatient distances but only compare sph and MM.
s_mm_dist<- test2_m %>% filter(S1 %in% c("LMM","RMM","LSPH","RSPH") & S2 %in% c("LMM","RMM","LSPH","RSPH")) %>%
  filter(P1==P2) %>% arrange(P1)

ggplot(s_mm_dist, aes(x=D1, y=value, fill=D1))+geom_boxplot()

anova(lmer(value ~ D1 + (1|P2), data=s_mm_dist))
anova(lm(value ~ D1, data=test))
...
intra3<-intra2 %>% filter(comp!="NvS")
mm_dist_plot<-ggplot(intra3, aes(x=comp, y=value, fill=D1))+geom_boxplot() +
  facet_grid(~D1,scales="free") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position = "none")+
  labs(x="Comparison", y="Weighted unifrac distance")

grid.arrange(mm_dist_plot, arrangeGrob(E1,E2,nrow=2,left="Average relative abundance (%)"), ncol=2)
grid.arrange(E1,E2,nrow=2, top="Unclassified Enterobacteriaceae relative abundance", left="Average relative abundance (%)")
grid.arrange(P1, arrangeGrob(P2,P3,ncol=1), ncol=2)
...
uwuf_intra2_CRS_S_M<-uwuf_intra2 %>% filter(comp!="NvS")
anova(lmer(value ~ comp + (1|P1), data=uwuf_intra2_CRS_S_M))
uwuf_intra2_CRS_N_M<-uwuf_intra2 %>% filter(comp!="SvS")
anova(lmer(value ~ comp + (1|P1), data=uwuf_intra2_CRS_N_M))
uwuf_intra2_CRS_S_N<-uwuf_intra2 %>% filter(comp!="MMvS")
anova(lmer(value ~ comp + (1|P1), data=uwuf_intra2_CRS_S_N))
#subset to control only and repeat
...
plot<-ggplot(cor_otu_data,aes(x=rel_abund, y=SNOT22))+geom_point(aes(colour=Patient))+
  geom_smooth(method=lm, fullrange=TRUE)+labs(x="relative abundnace (%)")+ylim(0,100)+facet_wrap(~OTU,7,6)

#Geom text looks bad because it's putting the text on multiple times from the data. Get new data frame with only one entry per family.
text_otu_data<-cor_otu_data %>% group_by(Family,Genus,OTU) %>% distinct(Genus, .keep_all=TRUE)
  
plot + geom_text(data=text_otu_data, aes(x=50, y=100,label=paste(Family,Genus)),size=2)
#Plot ind otus
indplot<-cor_otu_data %>% filter(OTU %in% c("410908","1075821")) %>%
  ggplot(aes(x=rel_abund, y=SNOT22)) + 
  geom_point(aes(color=Patient))+
  geom_smooth(method=lm)+
  labs(x="Relative Abundance %",y="SNOT-22")+
  ylim(0,100)+xlim(0,100)+facet_grid(.~Genus, scales="free")

text_otu<-cor_otu_data %>% filter(OTU %in% c("410908","1075821"))  %>% distinct(OTU, .keep_all=TRUE)

indplot2<-indplot +geom_text(data=text_otu,aes(x=50, y=100,label=OTU),size=3)
...
genera_sums<-cor_genera %>% group_by(Patient,Sample,Family,Genus) %>%
  summarise(gen_sums=sum(rel_abund),snot22=mean(SNOT22)) %>%
  filter(Patient!="NA")

gen_plot<-ggplot(genera_sums,aes(x=gen_sums, y=snot22))+geom_point(aes(colour=Patient))+
  geom_smooth(method=lm, fullrange=TRUE)+labs(x="relative abundance (%)")+ylim(0,100)+facet_wrap(~Family,3,3)

#Geom text looks bad because it's putting the text on multiple times from the data. Get new data frame with only one entry per family.
text_data<-genera_sums %>% group_by(Family) %>% distinct(Family, .keep_all = TRUE)
text_data2<-genera_best %>% group_by(Family) %>% distinct(Family, .keep_all = TRUE)

gen_plot+geom_text(data=text_data,aes(x=50, y=90,label=Genus),size=3)

#Only plot the interesting ones
genera_best<-genera_sums %>% filter(Genus %in% c("g__Streptococcus", "g__Corynebacterium", "g__Haemophilus") | (Family %in% c("f__Enterobacteriaceae","f__Neisseriaceae") & Genus=="g__"))

gen_best<-ggplot(genera_best,aes(x=gen_sums, y=snot22))+geom_point(aes(colour=Patient))+
  geom_smooth(method=lm, fullrange=TRUE)+labs(x="relative abundance (%)")+ylim(0,100)+facet_wrap(~Family,2,3)+labs(y="SNOT-22", x="Relative Abundance %")+ theme(legend.position='none')
gen_best2<-gen_best+geom_text(data=text_data2,aes(x=50, y=100,label=Genus),size=3)
grid.arrange(indplot2,gen_best2,ncol=2)
